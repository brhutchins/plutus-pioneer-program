...
mkPolicy pkh deadline ctx =
  traceIfFalse "Missing or wrong signature" (txSignedBy (scriptContextTxInfo ctx) pkh) &&
  traceIfFalse "After deadline"             (to deadline `contains` txInfoValidRange (scriptContextTxInfo ctx))
