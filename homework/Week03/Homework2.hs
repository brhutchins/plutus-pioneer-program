...
mkValidator pKeyHash slot () ctx =
    traceIfFalse "beneficary's signature missing" checkSig &&
  ...
  where
    ...
    checkSig :: Bool
    checkSig  = pKeyHash `elem` txInfoSignatories info
...
inst pKeyHash = Scripts.validator @Vesting
    ($$(PlutusTx.compile [|| mkValidator ||]) `PlutusTx.applyCode` PlutusTx.liftCode pKeyHash)
