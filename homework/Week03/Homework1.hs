...
mkValidator dat () ctx =
    (traceIfFalse "not beneficiary1's signature" (checkSig beneficiary1) && traceIfFalse "beneficiary1 can't grab after deadline" (not afterDeadline)) ||
    (traceIfFalse "not beneficiary2's signature" (checkSig beneficiary2) && traceIfFalse "beneficiary2 can't grab before deadline" (afterDeadline))
